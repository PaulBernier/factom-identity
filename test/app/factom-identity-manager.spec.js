...
describe('Factom identity management for applications', function() {
    xit('Should get active public identity keys', async function() {
        this.timeout(10000);
        const keysAtHeight = await manager.getActivePublicIdentityKeys(
            'e522a08e1697fd58c8d83fc72ceecc1efd8b8f12620e9f186d528f6e66ef6d42',
            58499
        );
        assert.deepEqual(keysAtHeight, [
            'idpub2kVac9L6ZiS91RrEoEfQrJGpcEoJJcGUyYE8zaq4gDB7VDhSjR',
            'idpub2pjBQgLnCcmoMWTZzqBQ2ShSEM4hJZNQZ1MJNtyhgBCrPBD7eo'
        ]);

        const keys = await manager.getActivePublicIdentityKeys(
            'e522a08e1697fd58c8d83fc72ceecc1efd8b8f12620e9f186d528f6e66ef6d42'
        );
    ...
    xit('Should check if key is active', async function() {
        ...
        assert.isTrue(
            await manager.isIdentityKeyActive(
                'e522a08e1697fd58c8d83fc72ceecc1efd8b8f12620e9f186d528f6e66ef6d42',
                'idpub2pjBQgLnCcmoMWTZzqBQ2ShSEM4hJZNQZ1MJNtyhgBCrPBD7eo',
                58499
            )
    ...
    it('Should create identity with existing keys', async function() {
        ...
        assert.equal(
            identityKeys[0].public,
            'idpub2Z2jmbQ4SnrpSYg7Eq9kJzDPcaeHYpqtkU32WU8fEHr386Kdtp'
        );
        assert.isUndefined(identityKeys[0].secret);
        assert.equal(
            identityKeys[1].public,
            'idpub3Doj5fqXye8PkX8w83hzPh3PXbiLhrxTZjT6sXmtFQdDyzwymz'
        );
