...
describe('Factom Walletd key store', function() {
    ...
    after(async function() {
        await Promise.all(
            IDENTITY_KEYS_TO_REMOVE.map(pub =>
                walletd.call('remove-identity-key', {
                    public: pub
                })
            )
        );
    ...
    it('Should get secret identity key', async function() {
        assert.equal(
            await store.getSecretIdentityKey(
                'idsec2Vn3VT8FdE1YpcDms8zSvXR4DGzQeMMdeLRP2RbMCSWCFoQDbS'
            ),
            'idsec2Vn3VT8FdE1YpcDms8zSvXR4DGzQeMMdeLRP2RbMCSWCFoQDbS'
        );
        assert.equal(
    ...
    it('Should get all identity keys', async function() {
        ...
        assert.includeDeepMembers(keys, [
            {
                public: 'idpub3Doj5fqXye8PkX8w83hzPh3PXbiLhrxTZjT6sXmtFQdDyzwymz',
                secret: 'idsec1wnZ9FLheMDXZNnnDHXdqZcMiDrgg2hTNzdseNLwFnEot362c4'
            },
            {
                public: 'idpub2Z2jmbQ4SnrpSYg7Eq9kJzDPcaeHYpqtkU32WU8fEHr386Kdtp',
                secret: 'idsec2CBtQMFqeMUdMQvDeXX91Aek8xLG8CfpeXnVBZkn8qVKvSqDkx'
            }
        ]);
    });
    ...
    it('Should remove multiple keys', async function() {
        ...
        await walletd.call('import-identity-keys', {
            keys: [{ secret: key1.secret }, { secret: key2.secret }]
        });
